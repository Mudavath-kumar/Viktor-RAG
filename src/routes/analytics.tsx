...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Verification accuracy, retrieval latency, top queries, and source coverage across your knowledge base." },
      { property: "og:title", content: "Analytics — Viktor RAG" },
      { property: "og:description", content: "Measure grounding, confidence, and retrieval performance." },
    ],
  }),
...
const kpis = [
  { label: "Verification accuracy", value: "97.2%", Icon: ShieldCheck, trend: "+0.6 pts vs last week" },
      ...
      <section className="px-6 max-w-[1200px] mx-auto pb-24">
        <div className="grid lg:grid-cols-12 gap-10 items-end">
          <div className="lg:col-span-7">
            ...
            <p className="mt-6 text-[#273C46] max-w-lg">
              Verification accuracy, retrieval latency, confidence distribution — observability for the parts of RAG that actually matter.
            </p>
          </div>
          <div className="lg:col-span-5">
            <img src={analyticsImg} alt="Hand-drawn line charts on tracing paper" loading="lazy" className="w-full rounded-3xl shadow-lg aspect-[4/3] object-cover" />
          </div>
        </div>
          ...
          {kpis.map((k) => (
            <div key={k.label} className="bg-white rounded-3xl p-6 border border-[#051A24]/5">
              <div className="flex justify-between items-start">
                <p className="text-xs text-[#273C46] uppercase tracking-wide max-w-[120px]">{k.label}</p>
                <k.Icon className="w-4 h-4 text-[#1f5d4f]" />
              </div>
              <p className="mt-4 font-mondwest text-[44px] leading-none text-[#051A24]">{k.value}</p>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 bg-[#051A24] rounded-3xl p-8 text-white">
            <p className="text-xs uppercase tracking-wide text-[#E0EBF0]">Confidence distribution · 30 days</p>
            <svg viewBox="0 0 400 160" className="mt-6 w-full h-44">
              ...
              <path d="M0,120 L20,110 L40,115 L60,90 L80,95 L100,72 L120,80 L140,55 L160,60 L180,42 L200,50 L220,30 L240,38 L260,28 L280,40 L300,45 L320,60 L340,75 L360,95 L380,115 L400,130 L400,160 L0,160 Z" fill="url(#g)" />
              <path d="M0,120 L20,110 L40,115 L60,90 L80,95 L100,72 L120,80 L140,55 L160,60 L180,42 L200,50 L220,30 L240,38 L260,28 L280,40 L300,45 L320,60 L340,75 L360,95 L380,115 L400,130" fill="none" stroke="#5cc9b1" strokeWidth="2" />
            </svg>
              ...
              {sources.map((s) => (
                <li key={s.name}>
                  <div className="flex justify-between text-sm mb-1">
                    <span>{s.name}</span><span className="font-mono text-[#273C46]">{s.pct}%</span>
                  </div>
